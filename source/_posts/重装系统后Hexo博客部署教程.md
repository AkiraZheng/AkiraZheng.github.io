...
# @enduml
# ```
# PlantUML 图片由 PlantUML 服务器在线渲染（无需安装 Java），图片会显示在文章中
#
# ⚠️ VPN/代理限制说明:
# 如果开启了 VPN（如 Clash），PlantUML 图片可能无法渲染，显示为裂开的图片
# 原因：VPN 会拦截所有流量，导致无法直接访问 www.plantuml.com 服务器
# 解决方法：在 VPN 中将 plantuml.com 添加到白名单/忽略代理列表
# Mac Clash 设置位置：
#   打开 Clash → 顶部菜单栏图标 → 设置 → "忽略这些主机与域的代理设置"
#   添加：plantuml.com 和 www.plantuml.com
# 或者临时关闭 VPN 再访问页面
```
