...
- **分类重复问题（如出现多个同名分类文件夹）：** Hexo 的 `db.json` 会缓存分类结构，修改 front matter 中的 categories 后必须先 `hexo clean` 再 `hexo g`，否则旧的分类结构会残留。此外，浏览器也会缓存旧页面，需用无痕模式或 `Cmd+Shift+R` 强制刷新。如果多篇文章想将同一个分类作为**平级独立分类**（而非父子嵌套），需使用数组语法 `- [分类名]`，每条数组是一条独立的层级路径。
